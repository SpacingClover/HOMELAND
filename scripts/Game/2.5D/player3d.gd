...
func e_interaction()->void:
	...
	for body : Node3D in area.get_overlapping_areas() + area.get_overlapping_bodies():
		...
		elif global_position.distance_to(body.global_position) < global_position.distance_to(closest.global_position) and (closest is RoomItemInstance and closest.is_interactable):
			closest = body
	if closest:
